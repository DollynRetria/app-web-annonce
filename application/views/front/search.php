            	...
            	<?php foreach($aListes as $annonceItem):?>
                    ...
                    <div class="bloc <?php echo $zClass ?>">
                        ...
                        <a href="<?php echo site_url('page/detail/' . $annonceItem['annonce_id'] )?>" class="img">
                        	<?php echo $controller->image($annonceItem['annonce_id'], $annonceItem['annonce_titre']);?>
                        </a>
                        <h3>
							<?php
                                
								$zTitre = wordwrap($annonceItem['annonce_titre'], 90, "<br />\n");
								echo ucfirst(strtolower(htmlentities($zTitre)));
                            ?>
                        </h3>
                        <p>
                        	<?php $zDesc =  truncate(strip_tags($annonceItem['annonce_description']), 90);?> 
							<?php $zDesc = str_replace($critere, "<b style=\"color:#FF5223\">" . $critere ."</b>", $zDesc);?>
                            <?php echo $zDesc?>
                        </p>
                        <a href="<?php echo site_url('page/detail/' . $annonceItem['annonce_id'] )?>" class="link"></a>
                        <span class="date" style="text-align:right">Publier le<br /><em><b><?php echo time2DatetimeFR(time($annonceItem['annonce_date']))?></b></em></span>
